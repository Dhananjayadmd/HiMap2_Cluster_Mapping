...
def main(C1_init_, C2_init_, cgra_cluster_r, cgra_cluster_c):
    try:
        ...
        num_of_columns = int(cgra_cluster_c)#4#4#5#4
        num_of_rows = int(cgra_cluster_r)#4#4#2#4
        row  = 0
        ...
        #for var in m.getVars():
            #print('%s %g' % (var.varName, var.x))
        #cluster_result = [[]] * number_of_cgra_clusters
        cluster_result = []
        ...
        cgra_clustering_outcome = "dfg_to_cgra_cluster_mapping_outcome.txt"
        i=0
